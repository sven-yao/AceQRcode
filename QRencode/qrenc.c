...
#include <stdlib.h>
#include <string.h>
#include "../3rdparty/pnglib/include/png.h"
...
// º”‘ÿlibpngÀ˘–Ëø‚
#ifdef _DEBUG
#pragma comment(lib, "../3rdparty/pnglib/lib/libpngd.lib")
#pragma comment(lib, "../3rdparty/pnglib/lib/zlibd.lib")
#else
#pragma comment(lib, "../3rdparty/pnglib/lib/libpng.lib")
#pragma comment(lib, "../3rdparty/pnglib/lib/zlib.lib")
#endif // _DEBUG
...
static char *optstring = "ho:l:s:v:m:d:t:Skci8MVf:b:";
...
static int color_set(unsigned int color[4], const char *value)
{
	...
	int count;
	if(len == 6) {
		count = sscanf_s(value, "%02x%02x%02x%n", &color[0], &color[1], &color[2], &len);
		if(count < 3 || len != 6) {
			return -1;
		}
		color[3] = 255;
	} else if(len == 8) {
		count = sscanf_s(value, "%02x%02x%02x%02x%n", &color[0], &color[1], &color[2], &color[3], &len);
		if(count < 4 || len != 8) {
			return -1;
		}
	} else {
...
static int writeSVG( QRcode *qrcode, const char *outfile )
{
	...
	_snprintf_s(fg, 7, "%02x%02x%02x", fg_color[0], fg_color[1],  fg_color[2]);
	_snprintf_s(bg, 7, "%02x%02x%02x", bg_color[0], bg_color[1],  bg_color[2]);
